...
def main():
    ...
    df = pd.read_csv(INCIDENTS_CSV, dtype=str)  # read all as string to avoid float GEOID problems
    ...
    # Normalize shapefile GEOIDs to string
    gdf[geoid_field] = gdf[geoid_field].astype(str).apply(normalize_geoid)

    # 3) compute centroids (in same CRS as shapefile; convert to EPSG:4326 for lat/lon)
    print("Computing centroids and reprojecting to EPSG:4326 (lat/lon)...")
    try:
        gdf_cent = gdf[[geoid_field, "geometry"]].copy()
    except KeyError:
        # if there are other required fields, just ensure geometry present
        gdf_cent = gdf.copy()[["geometry"]]
        gdf_cent[geoid_field] = gdf[geoid_field]

    # Compute centroid in shapefile CRS, then convert to WGS84
    ...
    # Prepare simple lookup table: GEOID -> lat/lon
