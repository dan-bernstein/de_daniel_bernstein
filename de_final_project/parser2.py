...
def parse_pdf(pdf_path, source_title):
    ...
    logging.info(f"Parsed {len(records)} cases from {pdf_path}")
    return records
...
def main():
    ...
    for idx, entry in enumerate(data["crime_news_pdfs"], start=1):
        ...
        logging.info(f"[{idx}/{len(data['crime_news_pdfs'])}] Processing PDF: {title}")
        download_pdf(url, pdf_path)
        records = parse_pdf(pdf_path, title)
        all_records.extend(records)
        logging.info(f"[{idx}/{len(data['crime_news_pdfs'])}] Total cases collected so far: {len(all_records)}")
